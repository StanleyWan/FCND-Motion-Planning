...
import numpy as np
from voroni_util import create_grid_and_edges
...
class MotionPlanning(Drone):
    ...
    def plan_path(self):    ## NEW
        ...
        if len(self.waypoints) > 0:
            ...
            # Stanley: No need.  We can use the self.global_position which provided from the Drone in the State_Callback. 
            ...
            #pruned_path = prune_waypoints(path)
            #print ("path is ", path)
            pruned_path = bres_prune(grid, path) 
            #pp = np.array(path)
            pp = np.array(pruned_path)
            plt.plot(pp[:,1], pp[:,0],'g', label="Pruned A Star Motion Path")
            plt.legend(loc='best')
            #plt.show()
            #fig.savefig('A Star Motion_pave.png', bbox_inches='tight')
            ...
            plt.plot([p1[1], p2[1]], [p1[0], p2[0]], 'b-', label='Voroni Map')
            plt.legend(loc='best')

            plt.xlabel('EAST')
            plt.ylabel('NORTH')
            #plt.show()
            fig.savefig('Voroni Map.png', bbox_inches='tight')
            print ("Voroni Map Saved")
            
            ## ***********  Do a Search on Voroni Map  *************
            ...
            for e in edges:
                ...
                #print ("p1, p2, dist = ", p1, p2, dist)
                G.add_edge(p1, p2, weight=dist)
                
                
            start_ne = grid_start
            goal_ne = grid_goal
            start_ne_g = closest_point_voroni(G, start_ne)
            goal_ne_g = closest_point_voroni(G, goal_ne)
            print("start_ne_g =", start_ne_g)
            print("goal_ne_g = ", goal_ne_g)
            
            path, cost = a_star_voroni(G, heuristic_voroni, start_ne_g, goal_ne_g)
            print(len(path))
            
            pruned_path_voroni = bres_prune(grid, path) 
            
            # equivalent to
            # plt.imshow(np.flip(grid, 0))
            plt.imshow(grid, origin='lower', cmap='Greys') 

            #  *** Draw the Voroni graph ***
            ...
            plt.plot([goal_ne[1], goal_ne_g[1]], [goal_ne[0], goal_ne_g[0]], 'r-',label="Voroni Motion Path")
            
            for i in range(len(pruned_path_voroni)-1):
                p1 = pruned_path_voroni[i]
                p2 = pruned_path_voroni[i+1]
                plt.plot([p1[1], p2[1]], [p1[0], p2[0]], 'y-')
            plt.plot([goal_ne[1], goal_ne_g[1]], [goal_ne[0], goal_ne_g[0]], 'y-', label="Pruned Voroni Motion Path")
            ...
            plt.legend(loc='best')
            #plt.show()
            fig.savefig('Voronoi Motion Path.png', bbox_inches='tight')
            print ("Graphic Done")

            
            # Convert path to waypoints
            waypoints = [[int(p[0] + north_offset), int(p[1] + east_offset), TARGET_ALTITUDE, 0] for p in pruned_path_voroni]
...
def heuristic_voroni(n1, n2):
    return LA.norm(np.array(n2) - np.array(n1))

def a_star_voroni(graph, h, start, goal):
...
def closest_point_voroni(graph, current_point):
    """
